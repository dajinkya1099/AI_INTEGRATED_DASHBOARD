...
#             elif line.strip().startswith("-"):
#                 # Example:
#                 # - id (integer) | Nullable: NO | Description: Primary key
                
#                 column_pattern = r"-\s*(\w+)\s*\((.*?)\)\s*\|\s*Nullable:\s*(YES|NO)\s*\|\s*Description:\s*(.*)"
#                 match = re.match(column_pattern, line.strip())
...
#             schema_text += f"\n\nTABLE: {table_name}\n"
#             schema_text += f"Description: {table_comment}\n"
#             schema_text += "Columns:\n"
#             current_table = table_name

#         schema_text += f" - {column_name} ({data_type}) | Nullable: {nullable} | Description: {column_comment}\n"
