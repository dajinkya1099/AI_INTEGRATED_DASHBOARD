"""
visualization_agent.py — AI Agent for Fast Dynamic Visualization
═══════════════════════════════════════════════════════════════════

ARCHITECTURE COMPARISON
────────────────────────────────────────────────────────────────
OLD  visualization_service.py  (Sequential):
  Request → Call1(20-40s) → wait → Call2(15-25s) → wait → Response
  Total: 35-65 seconds. Always both Ollama calls. No routing.

NEW  visualization_agent.py  (Smart Agent):
  Request
    │
    ▼
...
────────────────────────────────────────────────────────────────
  # OLD:
  from visualization_service import generate_react_visualization

  # NEW (drop-in replacement — same function name, same input/output):
  from visualization_agent import generate_react_visualization
...
────────────────────────────────────────────────────────────────
  ...
  "who / filter / show employees who"   → PYTHON_ONLY (no Ollama)
...
_FILTER_KW = {
    "who ", "which employee", "employees who", "employees with",
...
def tool_1_route(state: AgentState):
    ...
    # Filter/list keywords → Python only (table)
    if any(kw in q for kw in _FILTER_KW):
        state.strategy = Strategy.PYTHON_ONLY
