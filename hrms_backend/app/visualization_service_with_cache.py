...
# # Call 1 — Analyze intent, follow instructions, return JSON
# # qwen2.5:7b-instruct is best: instruction-tuned, accurate JSON output
# OLLAMA_MODEL_INTENT = "qwen2.5:7b-instruct"

# # Call 2 — Generate React / HTML code
# # qwen2.5-coder:3b is best: purpose-built for code, 2-3x faster than 7b
# # Install with:  ollama pull qwen2.5-coder:3b
# # Falls back to OLLAMA_MODEL_INTENT if not available
# OLLAMA_MODEL_CODE = "qwen2.5-coder:3b"

# ─────────────────────────────────────────────────────────────────────────────
# RESPONSE CACHE
# ─────────────────────────────────────────────────────────────────────────────
# Identical questions return instantly from cache.
# Key = MD5(schema + question).  TTL = 1 hour.  Max = 100 entries (LRU).
...
def _cache_get(key: str):
    ...
    if time.time() - entry["ts"] > _CACHE_TTL:
        del _cache[key]
        return None
    _cache.move_to_end(key)   # LRU: keep recently used at end
    return entry["value"]
...
def cache_clear():
    """Call this to clear the cache after data changes."""
    _cache.clear()
    print("[Cache] Cleared")
...
def generate_react_visualization(request: QueryRequest) -> dict:
    ...
    # ══════════════════════════════════════════════════════════
    # CACHE CHECK  — return instantly if same question seen before
    # ══════════════════════════════════════════════════════════
    ck = _cache_key(request.schemaName, request.textQue)
    cached = _cache_get(ck)
    if cached is not None:
        elapsed = round(time.time() - t_start, 3)
        print(f"  [Cache] HIT ✅  key={ck[:8]}  returned in {elapsed}s")
        cached["cache_hit"]      = True
        cached["response_time_s"] = elapsed
        return cached

    print(f"  [Cache] MISS — running Ollama pipeline  key={ck[:8]}")

    # ══════════════════════════════════════════════════════════
    # CALL 1  — Analyze + Structure  (model: OLLAMA_MODEL_INTENT)
    # ══════════════════════════════════════════════════════════
    t1 = time.time()
    ...
    print(f"\n▶ CALL 1 done in {t1_elapsed}s  (model: {OLLAMA_MODEL_INTENT})")
    print(f"  viz_type : {structured.get('viz_type')}")
    print(f"  title    : {structured.get('title')}")
    print(f"  x_key    : {structured.get('x_key')}  y_key: {structured.get('y_key')}")
    print(f"  rows     : {len(structured.get('data', []))}")
    print(f"  sample   : {structured.get('data', [])[:2]}")

    # ══════════════════════════════════════════════════════════
    # CALL 2  — Generate HTML  (model: OLLAMA_MODEL_CODE for charts)
    # ══════════════════════════════════════════════════════════
    t2 = time.time()
    ...
    call2_model = OLLAMA_MODEL_CODE if viz_type in ("bar","pie","line","area","scatter","radar") else "python"
    print(f"\n▶ CALL 2 done in {t2_elapsed}s  (model: {call2_model})")
    print(f"  HTML length: {len(html)}")

    total = round(time.time() - t_start, 2)
    print(f"\n  Total time: {total}s  (call1={t1_elapsed}s  call2={t2_elapsed}s)")
    ...
    result = {
        ...
        "structured":      structured,
        "cache_hit":       False,
        "response_time_s": total,
        "call1_model":     OLLAMA_MODEL_INTENT,
        "call2_model":     call2_model,
    }

    # ── Store in cache for next time ──────────────────────────────────────────
    _cache_set(ck, result)
    print(f"  [Cache] Stored  key={ck[:8]}  total_entries={len(_cache)}")
...
def call2_generate_html(structured: dict, schema_name: str, question: str) -> str:
    ...
    # Try code model first; if unavailable fall back to intent model
